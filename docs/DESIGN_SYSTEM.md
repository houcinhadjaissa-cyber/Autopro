...
*Status: ✅ OWNER-APPROVED | Primary Color: Custom Green (legally distinct) | Unified across ALL user types*
...
- MODIFIED AUTOPRO GREEN: #6FB81A (shifted +5° hue, +3% saturation)
- WHY: Legally distinct while keeping NVIDIA's "technical power" feel
- USAGE: Primary buttons, active states, success indicators, brand accents
- ACCESSIBILITY: Contrast ratio on white: 4.6:1 (WCAG AA), on black: 12.3:1 (WCAG AAA), Color-blind safe tested
...
- PRIMARY FONT (Display/Headings): "BinancePlex", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif → Fallback: System sans-serif
- NUMBERS FONT (Trading Data): "BinanceNova", ui-monospace, SFMono-Regular, Menlo, monospace → Fallback: System monospace
- ARABIC FONT (RTL Support): "Noto Sans Arabic", "BinancePlex", Tahoma, sans-serif + RTL layout + numeral shaping
- LEGAL NOTE: Using system font stacks avoids licensing; Implement with open-source: Primary:"Inter" (Google Fonts), Numbers:"JetBrains Mono" (open-source)
